...
void SplayTreeMap::zigZig(Node* x) {
    /*
	Made redundant by my over-the-top zigzag function
	*/
}
...
void SplayTreeMap::zigZag(Node* x) {
	...
	Node *a = x->parent;
	if (a) {
		if (z == a->left) a->left = x;
		else if (z == a->right) a->right = x;
	}
}
...
{
	Node *i = this->stmap->findSplay(id);
	

	if (&i[0] == NULL) {
		this->stmap->putSplay(id, amount);
	}
	else if (i[0].key != id) {
		this->stmap->putSplay(id, amount);
	}
